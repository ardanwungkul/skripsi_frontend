...
const form = ref({
    domain_name: props.order.domain_name,
    start_date: null,
    expired_date: null,
    vendor: null,
    user: null,
...
onMounted(async () => {
    await storeDomain.getDataCreate()
    data.value.user = storeDomain.create.user
    data.value.vendor = storeDomain.create.vendor
})
...
const handleAdd = async () => {
    ...
    formData.append('user_id', props.order.user.id)
    formData.append('todolist_id', props.todolist.id)
    ...
    <v-dialog max-width="500">
        <template v-slot:activator="{ props: activatorProps }">
            <button
                ...
                class="w-full px-3 py-1 bg-secondary-3 rounded-lg hover:bg-opacity-70 text-sm">
                Progress
            </button>
        </template>
