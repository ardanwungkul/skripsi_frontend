...
onBeforeMount(async () => {
    await store.getDataEdit(id.value)
    if (!store.edit) {
        router.replace('/404')
    } else {
        data.value.package = store.edit.package
        data.value.domain = store.edit.domain

        form.value.package_name = data.value.package.package_name
        form.value.package_description = data.value.package.package_description
        form.value.package_price = data.value.package.package_price
        form.value.package_hosting_storage =
            data.value.package.package_hosting_storage
        form.value.package_hosting_email =
            data.value.package.package_hosting_email
        form.value.package_support = data.value.package.package_support
        form.value.package_video_profile =
            data.value.package.package_video_profile
...
<template>
    <AdminLayout>
        <LoadingAdmin :isLoading="data.isLoading" />
